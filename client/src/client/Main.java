...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException, UnknownHostException, IOException {
		...
		Thread[] threads = new Thread[] {
			...
			new ClientConnectionThread(camera1monitor, "argus-3.student.lth.se", 22100, 20000),
			new EventDispatcher(camera1monitor, mw),
			new MotionDetector(camera1monitor, "http://argus-3.student.lth.se", 9095),
			new StatusDispatcher(camera1monitor,mw),
			new ClientSender(camera1monitor),
			new ClientReceiver(camera2monitor),
			new ClientConnectionThread(camera2monitor, "argus-1.student.lth.se", 22100, 20000),
			new EventDispatcher(camera2monitor, mw),
			new MotionDetector(camera2monitor, "http://argus-1.student.lth.se", 9095),
